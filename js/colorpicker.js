...
$('.btn').on('keyup', function(event) {
  ...
  if(event.keyCode == 27) { 
    var thisButton = $(this),
        thisGroup  = thisButton.parent('.btn-group');
...
function setupAnnounce(targetId, contentForAnnounce, timeoutOption) {
  if(!timeoutOption) { timeoutOption = 5000; }
  $('#' + targetId)
    .attr('aria-live', 'off')
    .html(contentForAnnounce);
  console.log('setupAnnounce fired');
  clearTimeout(countdownAnnouncer);
  countdownAnnouncer = setTimeout(function() {
    $('#' + targetId)
      .attr('aria-live', 'polite')
      .html(' ')
      .html(contentForAnnounce + ' ');
    console.log('countdownAnnouncer content fired')
  }, timeoutOption);
}

function setupSliderAnnounce(target, valueUpdate, contentForAnnounce, timeoutOption) {
  if(!timeoutOption) { timeoutOption = 2000 }
  target = $(target);
  console.log('setupSliderAnnounce fired');
  clearTimeout(sliderAnnouncer);
  target
      .attr('aria-valuenow', valueUpdate)
      .attr('aria-valuetext', ''); // insure delay of "{color} equals {value}" response
  sliderAnnouncer = setTimeout(function() {
    console.log('setupSliderAnnounce CONTENT fired');
    target
      .attr('aria-valuetext', contentForAnnounce);
  }, timeoutOption);
}
...
textfieldElements.change(function() {
  ...
  contrastUpdateString = contrastUpdateString.toFixed(2);
  ...
  // [ TASK : Create timing delay solution ]


  
  setupAnnounce('cp_a11y-status-update', contrastUpdateString, 3000); // note: will cancel if 1500 threshold isn't reached
...
$('.cp_color-settings_slide-marker, .cp_slider-textfield')
  .keydown( function( event ) { 
    ...
    stepNumber($(this),event);
  })

$('.cp_color-settings_slide-marker')
  .bind('focus', function() {
    var thisMarker = $(this);
    var thisFailRange = thisMarker.parent().find('.cp_color-settings_a11y-fail-range');
    if(!thisFailRange.hasClass('hide')) {
      var failTargetVal = Number(thisFailRange.attr('data-fail-value')), 
          failStartVal  = thisFailRange.attr('data-fail-start-value'),
          contentResult;
      
      if(failStartVal == 'zero') { contentResult = 'Contrast Fail Values are ' + failStartVal  + ' to ' + failTargetVal; }
      else {                       contentResult = 'Contrast Fail Values are ' + (255-failTargetVal+5) + ' to ' + failStartVal;  }

      setupAnnounce('cp_sr-only-announce', contentResult, 1500);
    }
  })
  .bind('keyup',  function(event) { 
    if(event.keyCode == 13) { // enter == 13
      setupAnnounce('cp_a11y-status-update', contrastUpdateString, 1500);
    }
  })
  .each(function() { 
    var self = $(this); 
...
function moveMarker( targetMarker, newValue, xPosition ) { // 
  ...
  if(newValue >= minValue && newValue <= maxValue) {
    ...
    var contentToAnnounce = targetMarker.attr('data-slider-text-before') + ' equals ' + newValue + targetMarker.attr('data-slider-text-after');

    targetMarker
      .css('transform', 'translate3d(' + xPosition + 'px, 0px, 0px)');
      // .attr('aria-valuenow', newValue);
    setupSliderAnnounce(targetMarker, newValue, contentToAnnounce, 1000);
...
function setRangesForUI( colorDataObj ) {
  if(colorDataObj.brighterColorIs == 'font' && colorDataObj.targetContrast != 0) {
    ...
    $('#cp_font-color-settings .cp_color-settings_a11y-fail-range').eq(0)
      ...
      .attr('data-fail-start-value', 'zero')
      .addClass('fail-range--flip')
    ...
    $('#cp_background-color-settings .cp_color-settings_a11y-fail-range').eq(0)
      ...
      .attr('data-fail-start-value', '255')
      .removeClass('fail-range--flip')
    ...
    $('.cp_color-settings_a11y-fail-range')
      ...
      .each(function() {
        var self = $(this);
        if(self.css('width') == '16px') self.addClass('hide'); // needed for all cases where no value applies.
      });
  }

  if(colorDataObj.brighterColorIs == 'bg' && colorDataObj.targetContrast != 0) {
    var bgFailRanges    = $('#cp_background-color-settings .cp_color-settings_a11y-fail-range');
    var fontFailRanges  = $('#cp_font-color-settings .cp_color-settings_a11y-fail-range');
    bgFailRanges.eq(0)
